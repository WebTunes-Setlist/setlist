...
const lyricsTabBtn = document.getElementById("tab-lyrics");
const otherTabs = document.querySelectorAll(".tab-btn:not(#tab-lyrics)");

lyricsTabBtn.addEventListener("click", () => {
  tabContainer.classList.add("lyrics-active");
});

otherTabs.forEach(btn => {
  btn.addEventListener("click", () => {
    tabContainer.classList.remove("lyrics-active");
  });
});
...
function showTab(id) {
  ...
  // ⭐ Remember which tab the user switched TO
  lastActiveTab = id;
  ...
  /* ---------------------------------------------------------
     TRACKS / RELATED TABS — restore your original behavior
  --------------------------------------------------------- */
  ...
  if (isVideo) {
    ...
    albumVideo.src = t.file;
    albumVideo.play().catch(() => {});
  } else {
    // Show image
    albumVideo.classList.add("hidden");
    albumImg.classList.remove("hidden");
    albumImg.src = t.cover || "";
  }
}
...
async function openAlbum(bandName, album, options = {}) {
  ...
  const restoreTab = options.restoreTab === true;   // ⭐ NEW OPTION
  ...
  tracks.forEach(file => {
    ...
    div.addEventListener("click", () => {
      ...
      if (isMobile()) {
        ...
        hideTrackVideo();
        handleTrackClick(albumPath, file, albumCover, bandName);
        showTab("tracks");
        return;
      }
      ...
      /* ============================================================
         DESKTOP LOGIC
      ============================================================ */
      if (isVideoTrack) hideTrackVideo();
  ...
  /* ============================================================
     ⭐ FINAL FIX — DO NOT FORCE TRACKS TAB
  ============================================================ */
  if (restoreTab) {
    showTab(lastActiveTab);   // ⭐ return to Tracks / Related / Lyrics
  } else {
    showTab("tracks");        // ⭐ default behavior unchanged
  }
}
...
function highlightCurrentTrackGlobal() {
  document.querySelectorAll(".track-item, .related-item").forEach(el => {
...
function handleTrackClick(albumPath, file, albumCover, bandName) {
  const audio = document.getElementById("audio-player");
  const playPauseBtn = document.getElementById("play-pause");

  const clickedId = albumPath + file;

  // Toggle if same track
  if (currentTrackId === clickedId) {
    if (isPlaying) {
      audio.pause();
      isPlaying = false;
      playPauseBtn.classList.remove("playing");   // footer icon sync
      highlightCurrentTrackGlobal();              // track icon sync
    } else {
      audio.play();
      isPlaying = true;
      playPauseBtn.classList.add("playing");      // footer icon sync
      highlightCurrentTrackGlobal();              // track icon sync
    }
    return;
  }
...
document.getElementById("footer-title").addEventListener("click", handleFooterJump);
...
function renderRelatedList(list) {
  ...
  list.forEach((track, index) => {
    ...
    div.addEventListener("click", () => {
      ...
      currentIndex = index;
      currentPlaylistSource = "related";   // ⭐ tells app this track came from Related tab
      lastActiveTab = "related";           // ⭐ ensures footer returns to Related tab
      ...
      if (isMobile()) {
        ...
        if (isVideoTrack) {
          currentVideoSrc = videoSrc;
          showTrackVideo(videoSrc);
          showTab("related");     // ⭐ stay on Related tab
          scrollVideoIntoView();
        } else {
          hideTrackVideo();
          handleTrackClick(track.path, track.file, track.albumCover, track.band);
          showTab("related");     // ⭐ stay on Related tab
        }
      ...
      highlightCurrentTrackGlobal();  // ⭐ highlight correct related track
    });
...
playPauseBtn.addEventListener("click", () => {
  ...
  // ⭐ Sync track icons in Tracks + Related
  highlightCurrentTrackGlobal();
});
